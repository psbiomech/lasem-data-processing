...
%   Prasanna Sritharan, June 2017
    ...
    % sheet header
    xldata(1,:) = {'Subject','Trial','Cohort','Affected','Trial Limb','File Path'};
    ...
    for s=1:length(subjs)
        ...
        for t=1:length(trials)
            if isempty(find(strcmpi(trials{t},{'cohort','affected'}),1))
                xldata(x,:) = {subjs{s}, trials{t}, bbstruct.(subjs{s}).cohort, bbstruct.(subjs{s}).affected, bbstruct.(subjs{s}).(trials{t}).triallimb, bbstruct.(subjs{s}).(trials{t}).filepath};
                x = x + 1;
            end
        end
    end
